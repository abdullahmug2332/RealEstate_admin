        ...
        if (type === "file") {
            const fileArray = Array.from(files).map((file) => {
                ...
                return {
                    ...
                    file, // original file (optional if you're uploading to backend)
                };
            });

            setFormData((prev) => ({
                ...prev,
                media: [...prev.media, ...fileArray], // append to existing media
            }));
        }
    };
    ...
    return (
        ...
        >
            <form onSubmit={handleSubmit} className="space-y-4  bg-white rounded-lg ">
                <h1 className='text-[40px] font-semibold color '>Add Property</h1>
                    ...
                    {formData.media.map((item, index) =>
                        item.type === "image" ? (
                            ...
                            <video
                                ...
                                src={item.src}
                                controls
                                className="w-24 h-24 rounded-[2px] object-cover"
                            />
                        ) : null
                    )}
                </div>
